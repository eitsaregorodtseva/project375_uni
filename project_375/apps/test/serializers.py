...
class PollSerializer(serializers.ModelSerializer):
    ...
    def is_user_poll_active(self):
        ...
        user_completed_polls = questionnaire.completed_polls.filter(user=user)

        if user_completed_polls:
            msg = f'Пользователь <{user}> ранее уже прошел опрос <{questionnaire.id}>'
            self.throw_exc(msg)
    ...
    def is_wrong_user_answer(self):
        ...
        user_polls = question.polls.filter(user=user)
        ...
        if not is_correct_answer:
            msg = f'Ответ ({user_answer}) пользователя <{user}> не соответствует вариантам ответа ' \
        ...
        if user_polls_count:
            ...
            if is_excessive_answer:
                msg = f'Пользователь <{user}> уже ответил на вопрос <{question.id}> в опросе ({questionnaire.id})'
                self.throw_exc(msg)

            # задублированный (не уникальный) ответ
            is_repeated_answer = bool(user_matched_polls_count)
            if is_repeated_answer:
                msg = f'Получен повторяющийся ответ пользователя <{user}> на вопрос <{question.id}> ' \
...
class CompletedPollSerializer(serializers.ModelSerializer):
    class Meta:
        model = CompletedPoll
        fields = ['id', 'user', 'questionnaire', 'completion_date']
