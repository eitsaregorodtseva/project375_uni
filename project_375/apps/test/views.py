...
class QuestionViewSet(ModelViewSet):
    ...
    def polls(self, request, pk=None):
        user = request.query_params.get('user', 0)
        question = Question.objects.get(pk=pk)
        polls = question.polls.filter(user_id=user) if user else question.polls.all()
...
class PollViewSet(ModelViewSet):
    ...
    def answers(self, request):
        user = request.query_params.get('user', 0)
        user_answers = Poll.objects.filter(user=user)
    ...
    def create(self, request, *args, **kwargs):
        ...
        polls = Poll.objects.filter(question__questionnaire=questionnaire).filter(user=user)
        ...
        for question in questions:
            ...
            completed_poll = CompletedPoll(user=user, questionnaire=questionnaire)
            completed_poll.save()
...
class CompletedPollViewSet(ModelViewSet):
    ...
    def by_user(self, request):
        user = request.query_params.get('user', 0)
        user_completed_polls = CompletedPoll.objects.filter(user=user)
